      ...
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        ...
        {room.phase === "lobby" ? (
          <>
            ...
            <MetricRow values={[{ label: "JOINED", value: String(members.length) }, { label: "ONLINE", value: String(members.filter((m) => m.connected).length) }, { label: "STATUS", value: "Open" }]} />
            <ParticipantList members={members} mode="lobby" />
            <ActionButton disabled={busy || members.length === 0} label={busy ? "Starting…" : "Start quiz  ⚡"} onPress={() => void run(() => startQuiz(session, room))} />
          </>
        ) : room.phase === "quiz" ? (
          <>

...
import { submitQuizAnswer } from "../lib/firebase/warRooms";
...
export function ParticipantQuizScreen({ onLeave, room, session }: { onLeave: () => void; room: WarRoomState; session: WarRoomSession }) {
  ...
  useEffect(() => { setSelected(null); setSubmitted(false); setError(null); }, [room.quiz.currentQuestionId]);
...
const styles = StyleSheet.create({
  ...
  submit: { alignItems: "center", backgroundColor: colors.accent, borderRadius: 13, justifyContent: "center", marginTop: 16, minHeight: 56 }, submitText: { color: colors.ink, fontSize: 15, fontWeight: "900" }, disabled: { opacity: 0.45 }, feedback: { backgroundColor: "rgba(72, 229, 232, 0.1)", borderRadius: 13, marginTop: 14, padding: 15 }, feedbackTitle: { color: colors.success, fontSize: 15, fontWeight: "900" }, feedbackBody: { color: colors.inkMuted, fontSize: 13, lineHeight: 19, marginTop: 5 },
